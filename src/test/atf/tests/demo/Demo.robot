...
*** Test Cases ***
MATUS_01
    Open Connection             ${DEV_00_HOST}              alias=SSH_00
    Login                       ${SSH_USER}                 ${SSH_PASS}
    
    ${APPIUM_00_PID}            Run Appium Remotely         SSH_00          ${DEV_00_PORT}      ${DEV_00_BOOTSTRAP_PORT}        
    ${APPIUM_01_PID}            Run Appium Remotely         SSH_00          ${DEV_01_PORT}      ${DEV_01_BOOTSTRAP_PORT}     
        
    Stop Appium Remotely        SSH_00                      ${APPIUM_00_PID}
    Stop Appium Remotely        SSH_00                      ${APPIUM_01_PID}
...
SMS_3G
    ...
    Send SMS        ${ms_a}         ${ms_b}         delivery_receipt=False
...
*** Keywords ***
Run Appium Remotely
    [arguments]                 ${SSH_alias}    ${PORT}     ${BOOTSTRAP_PORT}
    switch connection           ${SSH_alias}
    
    # run
    Write                       appium --port ${PORT} --bootstrap-port ${BOOTSTRAP_PORT} &
    Read                        delay=3s
    Write                       echo $!
    ${output}                   Read
    ${PID}                      Get PID     ${output}
    ...
    # check
    Write                               ps -e -o "pid cmd" | grep appium
    ${running_appium_processes}         Read                    delay=1s
    ${is_running}                       Is Running              ${running_appium_processes}        ${PID}
    Should be equal                     ${is_running}           0                                                       #0 if success, -1 otherwise

    [Return]                            ${PID}

Stop Appium Remotely
    [arguments]                         ${SSH_alias}    ${PID}
    switch connection                   ${SSH_alias}
    ...
    # stop    
    Run keyword if                      ${is_running}==0            Write         kill ${PID}
    ...                                 ELSE                        Log           Nothing to kill..

    # check
    Write                               ps -e -o "pid cmd" | grep appium   
    ${running_appium_processes}         Read                delay=1s
    ${is_running}                       Is Running          ${running_appium_processes}        ${PID}
    Should be equal                     ${is_running}       -1
